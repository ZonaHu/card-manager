...
async function testSmartCategorization() {
  ...
  try {
    ...
    for (const testCard of testCards) {
      ...
      const marker = isCorrect ? '[PASS]' : '[FAIL]';
      const categoryInfo = categories[predictedCategory];

      console.log(`${marker} "${testCard.name}"`);
      console.log(`   Expected: ${categories[testCard.expected]?.label}`);
      console.log(`   Predicted: ${categoryInfo?.label}`);
    ...
    if (accuracy >= 90) {
      ...
      console.log('Categorization needs improvement.');
    }
